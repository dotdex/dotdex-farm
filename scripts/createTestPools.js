...
async function createTestPool(name) {
  ...
  const _rewardToken = data.DotDexToken; //IBEP20

  if (data[name] == null) throw "expected " + name;
  ...
  const SousPool = await DotDexStakingFactory.deployPool(
    _stakedToken,
    _rewardToken,
    _rewardPerSecond,
    _startTimestamp,
  ...
  save(chainId, name + "_SousChef", {
    address: event.args.sousChef,
